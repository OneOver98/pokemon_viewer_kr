...
def parse_pvpoke_data():
    ...
    for line in lines:
        ...
        if len(parts) == 5:
            ...
            # Use lowercase pvpoke_id for export string
            export_id = pvpoke_id
            if is_shadow and not export_id.endswith("-shadow"):
                export_id += "-shadow"
            export_str = f"{export_id},{','.join(moves)}"
    ...
    os.makedirs('pvpoke-viewer_v3', exist_ok=True)
    with open('pvpoke-viewer_v3/data.json', 'w', encoding='utf-8') as f:
        json.dump(list(players.values()), f, ensure_ascii=False, indent=2)
        
    print(f"Successfully exported {len(players)} players to pvpoke-viewer_v3/data.json")
